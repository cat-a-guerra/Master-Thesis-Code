function area_pos = positive_area(x,y)
        ...
        y_pos=y.*pos;                   % Returns the y elements in which y>0

        x_pos=x_pos(x_pos>0);           % Vector with every x which corresponds to y positive values
        y_pos=y_pos(y_pos>0);           % Vector with every y positive value
        
        if pos(1)==1                    % When x=0 is positive
            x_pos=[0; x_pos];
        end
        ...
        if pos(1)==0                    % When x=0 is negative
            x(x==0)=[];
