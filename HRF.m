...
T2DM_Thr_betas = cell(n_T2DM*2*n_points,n_rois+1);
T2DM_Sub_betas = cell(n_T2DM*2*n_points,n_rois+1);

T2DM_Thr_SE = cell(n_T2DM*2*n_points,n_rois+1);
T2DM_Sub_SE = cell(n_T2DM*2*n_points,n_rois+1);

CNT_Thr_betas = cell(n_CNT*2*n_points,n_rois+1);
CNT_Sub_betas = cell(n_CNT*2*n_points,n_rois+1);

CNT_Thr_SE = cell(n_CNT*2*n_points,n_rois+1);
CNT_Sub_SE = cell(n_CNT*2*n_points,n_rois+1);
...
for c = 1:n_CNT
    CNT_Thr_betas((1+16*(c-1):16+16*(c-1)),:) = [ROIbetas(2065+((c-1)*32): 2072+((c-1)*32),:); ROIbetas(2049+((c-1)*32): 2056+((c-1)*32),:)];
    CNT_Sub_betas((1+16*(c-1):16+16*(c-1)),:) = [ROIbetas(2073+((c-1)*32): 2080+((c-1)*32),:); ROIbetas(2057+((c-1)*32): 2064+((c-1)*32),:)];
    CNT_Thr_SE((1+16*(c-1):16+16*(c-1)),:) = [ROISEs(2065+((c-1)*32): 2072+((c-1)*32),:); ROISEs(2049+((c-1)*32): 2056+((c-1)*32),:)];
    CNT_Sub_SE((1+16*(c-1):16+16*(c-1)),:) = [ROISEs(2073+((c-1)*32): 2080+((c-1)*32),:); ROISEs(2057+((c-1)*32): 2064+((c-1)*32),:)];    
end
...
for t = 1:n_T2DM
    T2DM_Thr_betas((1+16*(t-1):16+16*(t-1)),:) = [ROIbetas(17+((t-1)*32): 24+((t-1)*32),:); ROIbetas(1+((t-1)*32): 8+((t-1)*32),:)];
    T2DM_Sub_betas((1+16*(t-1):16+16*(t-1)),:) = [ROIbetas(25+((t-1)*32): 32+((t-1)*32),:); ROIbetas(9+((t-1)*32): 16+((t-1)*32),:)];
    T2DM_Thr_SE((1+16*(t-1):16+16*(t-1)),:) = [ROISEs(17+((t-1)*32): 24+((t-1)*32),:); ROISEs(1+((t-1)*32): 8+((t-1)*32),:)];
    T2DM_Sub_SE((1+16*(t-1):16+16*(t-1)),:) = [ROISEs(25+((t-1)*32): 32+((t-1)*32),:); ROISEs(9+((t-1)*32): 16+((t-1)*32),:)];
end
...
for r = 1:length(ROI_regions)
    headers_ROIs{r} = char(strrep(strcat(ROI_regions(r)),' ','_'));
end

% Every raw data in each ROI per condition and group
raw_data_CNT_Thr_betas = array2table(CNT_Thr_betas(:,2:end),'VariableNames',headers_ROIs);
raw_data_CNT_Thr_SE = array2table(CNT_Thr_SE(:,2:end),'VariableNames',headers_ROIs);
raw_data_CNT_Sub_betas = array2table(CNT_Sub_betas(:,2:end),'VariableNames',headers_ROIs);
raw_data_CNT_Sub_SE = array2table(CNT_Sub_SE(:,2:end),'VariableNames',headers_ROIs);

raw_data_T2DM_Thr_betas = array2table(T2DM_Thr_betas(:,2:end),'VariableNames',headers_ROIs);
raw_data_T2DM_Thr_SE = array2table(T2DM_Thr_SE(:,2:end),'VariableNames',headers_ROIs);
raw_data_T2DM_Sub_betas = array2table(T2DM_Sub_betas(:,2:end),'VariableNames',headers_ROIs);
raw_data_T2DM_Sub_SE = array2table(T2DM_Sub_SE(:,2:end),'VariableNames',headers_ROIs);
...
table_raw_CNT_Thr_betas = [headers_CNT raw_data_CNT_Thr_betas];
table_raw_CNT_Thr_SE = [headers_CNT raw_data_CNT_Thr_SE];
table_raw_CNT_Sub_betas = [headers_CNT raw_data_CNT_Sub_betas];
table_raw_CNT_Sub_SE = [headers_CNT raw_data_CNT_Sub_SE];

table_raw_T2DM_Thr_betas = [headers_T2DM raw_data_T2DM_Thr_betas];
table_raw_T2DM_Thr_SE = [headers_T2DM raw_data_T2DM_Thr_SE];
table_raw_T2DM_Sub_betas = [headers_T2DM raw_data_T2DM_Sub_betas];
table_raw_T2DM_Sub_SE = [headers_T2DM raw_data_T2DM_Sub_SE];
...
mean_per_subject_thr_CNT = zeros(n_points,n_CNT);
mean_per_subject_sub_CNT = zeros(n_points,n_CNT);
mean_per_subject_thr_T2DM = zeros(n_points,n_T2DM);
mean_per_subject_sub_T2DM = zeros(n_points,n_T2DM);

total_avg_thr_CNT = zeros(n_points,n_rois);
total_avg_thr_T2DM = zeros(n_points,n_rois);
total_std_thr_CNT = zeros(n_points,n_rois);
total_std_thr_T2DM = zeros(n_points,n_rois);

total_avg_sub_CNT = zeros(n_points,n_rois);
total_avg_sub_T2DM = zeros(n_points,n_rois);
total_std_sub_CNT = zeros(n_points,n_rois);
total_std_sub_T2DM = zeros(n_points,n_rois);

total_median_thr_CNT = zeros(n_points,n_rois);
total_median_thr_T2DM = zeros(n_points,n_rois);
total_iqr_thr_CNT = zeros(n_points,n_rois);
total_iqr_thr_T2DM = zeros(n_points,n_rois);

total_median_sub_CNT = zeros(n_points,n_rois);
total_median_sub_T2DM = zeros(n_points,n_rois);
total_iqr_sub_CNT = zeros(n_points,n_rois);
total_iqr_sub_T2DM = zeros(n_points,n_rois);

total_average_HRF_parameters_thr_CNT = zeros(11,n_rois);    % 11 since we have 11 parameters of interest
total_average_HRF_parameters_thr_T2DM = zeros(11,n_rois);
total_median_HRF_parameters_thr_CNT = zeros(11,n_rois);
total_median_HRF_parameters_thr_T2DM = zeros(11,n_rois);

total_average_HRF_parameters_sub_CNT = zeros(11,n_rois);
total_average_HRF_parameters_sub_T2DM = zeros(11,n_rois);
total_median_HRF_parameters_sub_CNT = zeros(11,n_rois);
total_median_HRF_parameters_sub_T2DM = zeros(11,n_rois);
...
for r=2:n_rois+1
    for i=1:n_subjects
        ...
        if i<65                                                             % until subject 65 --> T2DM 
            mean_per_subject_thr_T2DM(:,i) = mean_thr;                      % every mean value of beta per T2DM subject in the Thr condition for each ROI 
            mean_per_subject_sub_T2DM(:,i) = mean_sub;
        else                                                                % after subject 65 --> CNT
            mean_per_subject_thr_CNT(:,i-n_T2DM) = mean_thr;                % every mean value of beta per CNT subject in the Thr condition for each ROI 
            mean_per_subject_sub_CNT(:,i-n_T2DM) = mean_sub;
        end
    ...
    total_avg_thr_CNT(:,r-1) = avg_thr_CNT;                                 % stores every average beta values per datapoint of CNT subjects in the Thr condition
    total_avg_thr_T2DM(:,r-1) = avg_thr_T2DM;                               % stores every average beta values per datapoint of T2DM subjects in the Thr condition
    total_std_thr_CNT(:,r-1) = std_thr_CNT;                                 % stores every standard deviation of the beta values over each datapoint of CNT subjects in the Thr condition
    total_std_thr_T2DM(:,r-1) = std_thr_T2DM;                               % stores every standard deviation of the beta values over each datapoint of T2DM subjects in the Thr condition
    
    total_median_thr_CNT(:,r-1) = median_thr_CNT;                           % stores every median beta values per datapoint of CNT subjects in the Thr condition
    total_median_thr_T2DM(:,r-1) = median_thr_T2DM;                         % stores every median beta values per datapoint of T2DM subjects in the Thr condition
    total_iqr_thr_CNT(:,r-1) = iqr_thr_CNT;                                 % stores every interquartile range of the beta values over each datapoint of CNT subjects in the Thr condition
    total_iqr_thr_T2DM(:,r-1) = iqr_thr_T2DM;                               % stores every interquartile range of the beta values over each datapoint of T2DM subjects in the Thr condition
        ...
        total_average_HRF_parameters_thr_CNT(:,r-1) = HRF_parameters;
        ...
        HRF_parameters = [peak; peak_latency; slope_to_peak; total_area; pos_area; first_pos_area; second_pos_area; third_pos_area; neg_area; initial_dip_area; undershoot_area];
        total_median_HRF_parameters_thr_CNT(:,r-1) = HRF_parameters;
        ...
        HRF_parameters = [peak; peak_latency; slope_to_peak; total_area; pos_area; first_pos_area; second_pos_area; third_pos_area; neg_area; initial_dip_area; undershoot_area];
        total_average_HRF_parameters_thr_T2DM(:,r-1) = HRF_parameters;
        ...
        HRF_parameters = [peak; peak_latency; slope_to_peak; total_area; pos_area; first_pos_area; second_pos_area; third_pos_area; neg_area; initial_dip_area; undershoot_area];
        total_median_HRF_parameters_thr_T2DM(:,r-1) = HRF_parameters;
    ...
    total_avg_sub_CNT(:,r-1) = avg_sub_CNT;         
    total_avg_sub_T2DM(:,r-1) = avg_sub_T2DM;      
    total_std_sub_CNT(:,r-1) = std_sub_CNT;            
    total_std_sub_T2DM(:,r-1) = std_sub_T2DM;           
    
    total_median_sub_CNT(:,r-1) = median_sub_CNT;         
    total_median_sub_T2DM(:,r-1) = median_sub_T2DM;      
    total_iqr_sub_CNT(:,r-1) = iqr_sub_CNT;                  
    total_iqr_sub_T2DM(:,r-1) = iqr_sub_T2DM;               
        ...
        HRF_parameters = [peak; peak_latency; slope_to_peak; total_area; pos_area; first_pos_area; second_pos_area; third_pos_area; neg_area; initial_dip_area; undershoot_area];
        total_average_HRF_parameters_sub_CNT(:,r-1) = HRF_parameters;
        ...
        HRF_parameters = [peak; peak_latency; slope_to_peak; total_area; pos_area; first_pos_area; second_pos_area; third_pos_area; neg_area; initial_dip_area; undershoot_area];
        total_median_HRF_parameters_sub_CNT(:,r-1) = HRF_parameters;
        ...
        HRF_parameters = [peak; peak_latency; slope_to_peak; total_area; pos_area; first_pos_area; second_pos_area; third_pos_area; neg_area; initial_dip_area; undershoot_area];
        total_average_HRF_parameters_sub_T2DM(:,r-1) = HRF_parameters;
        ...
        HRF_parameters = [peak; peak_latency; slope_to_peak; total_area; pos_area; first_pos_area; second_pos_area; third_pos_area; neg_area; initial_dip_area; undershoot_area];
        total_median_HRF_parameters_sub_T2DM(:,r-1) = HRF_parameters;       
...
total_peak_thr_CNT = zeros(n_CNT,n_rois);
total_peak_thr_T2DM = zeros(n_T2DM,n_rois);
total_peak_latency_thr_CNT = zeros(n_CNT,n_rois);
total_peak_latency_thr_T2DM = zeros(n_T2DM,n_rois);

total_peak_sub_CNT = zeros(n_CNT,n_rois);
total_peak_sub_T2DM = zeros(n_T2DM,n_rois);
total_peak_latency_sub_CNT = zeros(n_CNT,n_rois);
total_peak_latency_sub_T2DM = zeros(n_T2DM,n_rois);
    
CV_peak_thr_CNT = zeros(1,n_rois);
CV_peak_thr_T2DM = zeros(1,n_rois);
CV_peak_latency_thr_CNT = zeros(1,n_rois);
CV_peak_latency_thr_T2DM = zeros(1,n_rois);

CV_peak_sub_CNT = zeros(1,n_rois);
CV_peak_sub_T2DM = zeros(1,n_rois);
CV_peak_latency_sub_CNT = zeros(1,n_rois);
CV_peak_latency_sub_T2DM = zeros(1,n_rois);
...
for r=2:(n_rois+1)
    for i=1:n_subjects
        ...
        if i<65                                                                         % until subject 65 --> T2DM
            ...
            total_peak_thr_T2DM(i,r-1) = peak_thr_T2DM;                                 % stores every HRF peak amplitude value of T2DM subjects in the Thr condition per ROI 
            total_peak_latency_thr_T2DM(i,r-1) = peak_latency_thr_T2DM*2.5;             % stores every HRF peak latency value of T2DM subjects in the Thr condition per ROI (We multiply by 2.5 to get the time. We don't need to subtract -1 due to the aforementioned reasons)
            total_peak_sub_T2DM(i,r-1) = peak_sub_T2DM;
            total_peak_latency_sub_T2DM(i,r-1) = peak_latency_sub_T2DM*2.5;
        else                                                                            % after subject 65 --> CNT
            ...
            total_peak_thr_CNT(i-n_T2DM,r-1) = peak_thr_CNT;                            % stores every HRF peak amplitude value of CNT subjects in the Thr condition per ROI
            total_peak_latency_thr_CNT(i-n_T2DM,r-1) = peak_latency_thr_CNT*2.5;        % stores every HRF peak latency value of CNT subjects in the Thr condition per ROI (We multiply by 2.5 to get the time. We don't need to subtract -1 due to the aforementioned reasons)
            total_peak_sub_CNT(i-n_T2DM,r-1) = peak_sub_CNT;
            total_peak_latency_sub_CNT(i-n_T2DM,r-1) = peak_latency_sub_CNT*2.5;
        end
...
for r=1:22
    ...
    CV_peak_thr_CNT(1,r) = CV_peak;
    CV_peak_latency_thr_CNT(1,r) = CV_latency;
    ...
    CV_peak_sub_CNT(1,r) = CV_peak;
    CV_peak_latency_sub_CNT(1,r) = CV_latency;
    ...
    CV_peak_thr_T2DM(1,r) = CV_peak;
    CV_peak_latency_thr_T2DM(1,r) = CV_latency;
    ...
    CV_peak_sub_T2DM(1,r) = CV_peak;
    CV_peak_latency_sub_T2DM(1,r) = CV_latency;
end
