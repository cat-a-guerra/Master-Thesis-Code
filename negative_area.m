function area_neg = negative_area(x,y)
        ...
        if isempty(idx)                                                              % No intersection
           x_intersect=[];
        else
            if idx(end)==8
                for j = 1:numel(idx)-1
        ...
        y_neg=y.*neg;                   % Returns the y elements in which y<0
        
        x_neg=x_neg(x_neg>0);           % Vector with every x which corresponds to y negative values
        y_neg=y_neg(y_neg<0);           % Vector with every y negative value

        if neg(1)==1                    % When x=0 is negative
            x_neg=[0; x_neg];
        end
        ...
        if neg(1)==0                    % When x=0 is positive
            x(x==0)=[];
