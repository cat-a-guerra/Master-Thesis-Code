...
hrf = fmridesign(time,0,[1 0],[],hrf_parameters);
...
% we get the coefficients of a quadratic function which could fit well into 
% our original HRF. 
...
id_hrf = [initial_dip_function; hrf(23:end)];                               % the initial dip HRF will be composed by the initial dip section (0 to 2.1s) and the original HRF from 2.2 s onwards


% Adjusting the quadratic function and the original HRF slopes so that the 
% final HRF appears more homogeneous by using a linear function. 
...
figure
total_area = area(time, id_hrf, min(id_hrf), 'FaceColor', [0.4 0.78 0.94]);         % patching the area under the initial dip HRF down to its minimum
...
negative_time = negative_time(negative_time>0);                                     % gets every time value which corresponds to negative initial dip HRF's values
negative_hrf = negative_hrf(negative_hrf<0);                                        % gets every negative initial dip HRF value
...
% begins (t=0s, according to our HRF) and contains the values of HRF and 
% y=0 at the time window corresponding to the initial dip.
...
initial_dip_hrf = [negative_hrf(negative_time<=peak_latency); 0];                               % gets the negative values of the HRF that were prior to the peak latency and the zero from the initial dip to rise peak transition
...
if isequal(time(negative_time<=peak_latency),negative_time(negative_time<=peak_latency))==1     
    initial_dip_time_y0 = flipud(negative_time(initial_dip_time<roots(1)));                     % retrieves again the time points of the negative values of the HRF that were prior to the peak latency and flips them up to down
else
    initial_dip_time_y0 = flipud([roots(1); negative_time(initial_dip_time<roots(2))]);         % retrieves again the time points of the negative values of the HRF that were prior to the peak latency but including the instant of the first intersection between the HRF and y=0 (in this case, 0) and flips them up to down                                
end
...
if isequal(time(negative_time<=peak_latency),negative_time(negative_time<=peak_latency))==1     
    ...
    undershoot_hrf = [0; negative_hrf(negative_time>roots(2))];                                 % gets the zero from the peak fall -> undershoot transition and the negative values of the HRF that were posterior to that instant
else
...
% [beginning of undershoot; end of undershoot[ s
if isequal(time(negative_time<=peak_latency),negative_time(negative_time<=peak_latency))==1     
    undershoot_time_y0 = flipud(negative_time(negative_time>roots(2)));                         % retrieves again the time points of the negative values of the HRF that were posterior to the rise fall -> undershoot transition zero and flips them up to down
else
...
positive_time = positive_time(positive_time>0);                                     % gets every time value which corresponds to positive initial dip HRF's values
...
first_positive_hrf = positive_hrf(positive_time<=5);                                % gets every time value which corresponds to the first positive HRF section's values
first_positive_time = positive_time(positive_time<=5);                              % gets every first positive HRF section value
...
% that instant up to 5s. 

% Perimeter part which contains the HRF values and time values from 
% [end of initial dip; 5] s
if isempty(time(positive_time<roots(1)))==1 || isempty(time(negative_time<roots(1)))==0         % if there is not a positive HRF beginning or if the HRF is not always negative until the initial dip   
    if roots(2)<5                                                                               % if the initial dip ends before the time point of 5s
        up_to_five_time = [roots(2); first_positive_time(first_positive_time>roots(2))];        % gets the end of initial dip -> rise to peak transition zero - it is the 2nd intersection of the HRF with y=0 in this condition - and the time points of the positive HRF values from that instant to 5s
        up_to_five_hrf = [0; first_positive_hrf(first_positive_time>roots(2))];                 % gets the zero from the end of initial dip -> rise to peak transition and the positive values of the HRF from that instant to 5s
    end
else
    if roots(1)<5
        up_to_five_time = [roots(1); first_positive_time(first_positive_time>roots(1))];        % gets the end of initial dip -> rise to peak transition zero - it is the 1st intersection of the HRF with y=0 if there is a positive HRF beginning or an always negative HRF until the initial dip - and the time points of the positive HRF values from that instant to 5s
...
if isempty(time(positive_time<roots(1)))==1 || isempty(time(negative_time<roots(1)))==0         
    if roots(2)<5                                                                               
        up_to_five_time_y0 = flipud(first_positive_time(first_positive_time>roots(2)));         % retrieves again the time points of the positive values of the HRF that were posterior to the initial dip and before 5s and flips them up to down       
    end
else
    if roots(1)<5
...
% [5; 10] s
if isempty(time(positive_time<roots(1)))==1 || isempty(time(negative_time<roots(1)))==0         
    if roots(3)<10                                                                              % if the peak fall ends before the time point of 10s
        five_to_ten_time = [second_positive_time(second_positive_time<roots(3)); roots(3)];     % gets the time points of the positive HRF values from 5s to the end of peak fall -> undershoot transition zero - it is the 3rd intersection of the HRF with y=0 in this condition 
        five_to_ten_hrf = [second_positive_hrf(second_positive_time<roots(3)); 0];              % gets the positive values of the HRF from 5s to the end of peak fall -> undershoot transition zero              
    else                                                                                        % if the peak fall ends after the time point of 10s
        five_to_ten_time = [second_positive_time(second_positive_time<10); 10];                 % gets the time points of the positive HRF values from 5s to 10s 
        five_to_ten_hrf = [second_positive_hrf(second_positive_time<10); 0];                    % gets the positive values of the HRF from 5s to 10s
    end
else
    if roots(2)<10
...
if isempty(time(positive_time<roots(1)))==1 || isempty(time(negative_time<roots(1)))==0         
    if roots(3)<10                                                                              
        five_to_ten_time_y0 = flipud(second_positive_time(second_positive_time<roots(3)));      % retrieves again the time points of the positive values of the HRF from 5s to the end of peak fall -> undershoot transition zero - it is the 3rd intersection of the HRF with y=0 in this condition and flips them up to down  
    else                                                                                        
        five_to_ten_time_y0 = flipud(second_positive_time(second_positive_time<10));            % retrieves again the time points of the positive values of the HRF from 5s to 10s and flips them up to down  
    end
else
    if roots(2)<10
...
% We close the (5s -> 10s -> 5s) loop in time and values and patch it
...
% of a closed loop which starts and ends at 10s, and contains the values of 
% HRF and y=0 at the time window from 10 to 17.5 s (the end of our HRF).
...
if isempty(third_positive_hrf)==0 && isempty(third_positive_time)==0                            % if from 10 to 17.5s there are positive HRF values (the peak fall -> undershoot transition zero takes place in this time window)
    
    % Perimeter part which contains the HRF values and time values from 
    % [10; 17.5] s
    if isempty(time(positive_time<roots(1)))==1 || isempty(time(negative_time<roots(1)))==0         
        if roots(3)>10 
            ten_to_end_time = [third_positive_time(third_positive_time<roots(3)); roots(3)];     % gets the time points of the positive HRF values from 10s to the end of peak fall -> undershoot transition zero - it is the 3rd intersection of the HRF with y=0 in this condition 
            ten_to_end_hrf = [third_positive_hrf(third_positive_time<roots(3)); 0];              % gets the positive values of the HRF from 10s to the end of peak fall -> undershoot transition zero              
        end
    else
        if roots(2)>10
             ten_to_end_time = [third_positive_time(third_positive_time<roots(2)); roots(2)];     % gets the time points of the positive HRF values from 10s to the end of peak fall -> undershoot transition zero - it is the 2rd intersection of the HRF with y=0 in this condition 
             ten_to_end_hrf = [third_positive_hrf(third_positive_time<roots(2)); 0];              % gets the positive values of the HRF from 10s to the end of peak fall -> undershoot transition zero              
        end
    end
    ...
    if isempty(time(positive_time<roots(1)))==1 || isempty(time(negative_time<roots(1)))==0         
        if roots(3)>10                                                                              
            ten_to_end_time_y0 = flipud(third_positive_time(third_positive_time<roots(3)));      % retrieves again the time points of the positive values of the HRF from 5s to the end of peak fall -> undershoot transition zero - it is the 3rd intersection of the HRF with y=0 in this condition and flips them up to down  
        end
    else
        if roots(2)>10
    ...
    % We close the (10s -> 17.5s -> 10s) loop in time and values and patch 
    % it
...
hold on 
set(gca,'XTick',0:2.5:17.5);                                                    % time range: [0; 17.5]s with a 2.5s step
xlabel('Time (seconds)')
ylabel('Intensity')
